...
export class FilmListService {
  ...
  filmList$: BehaviorSubject<Array<any>> = new BehaviorSubject<Array<any>>([]);
  ...
  findByName(value: string) {
    let url = `./api/films?title_like=${value}`;
    this.http.get(url, { observe: 'response' }).subscribe((data: any) => {
      if (data.body.length > 0) {
        this.findItems$.next(this.transformFindResult(data.body));
      } else {
        let url = `./api/films?titleOrigin_like=${value}`;
        this.http.get(url, { observe: 'response' }).subscribe((data: any) => {
          this.findItems$.next(this.transformFindResult(data.body));
        });
      }
  ...
  private transformFindResult(data: Array<any>) {
    ...
    data.forEach((element: any) => {
      const newItem = {
        value: element.title,
        link: 'film/' + element.id,
        img: './images/posters/' + element.poster.name + '.jpeg',
        text: element.title + ' | ' + element.titleOrigin,
      };
      findResult.push(newItem);
    });
    this.findItems$.next(findResult);
    return findResult;
  }
}
